...
`comp_latch` once to trigger the comparator.  It reads `q` on the
...
propagate `pwm_out` and `comp_latch` changes to SPICE instantly, and
...
module sar_controller #(
    ...
    output reg               done,
    output reg               comp_latch  // pulses once per bit
);
```
...
3. Pulses `comp_latch` to trigger the comparator
...
With a 10 MHz SAR clock and `SETTLE_CYCLES = 50`, each bit takes
5 μs = 5τ to settle, ensuring the DAC output reaches a stable value.
...
module adc #(
    ...
    wire pwm_out, q, qb, comp_latch;

    // SAR clock: clk / SAR_DIV
    // comp_latch: generated by SAR controller (once per bit)

    pwm_dac u_analog (                        // SPICE stub
        .pwm_in(pwm_out), .clk(comp_latch),
        .vin(vin), .q(q), .qb(qb)
    );
...
comparator is latched by `comp_latch` from the SAR controller —
...
module tb_pwm_dac;
    ...
    adc #(.N_BITS(8), .SAR_DIV(100), .SETTLE_CYCLES(50)) dut (
        .clk(clk), .reset_n(reset_n),
...
- **`clk`** (1 GHz): Fast clock for the PWM counter.  With 8 bits,
  ...
  `SETTLE_CYCLES = 50` → 5 μs (= 5τ) settling per bit.
...
# Start single fast clock (SAR clock + comp_latch derived internally)
...
| File | Contents |
|------|----------|
| `tb_pwm_dac.vcd` | Digital signals: `value[7:0]`, `done`, `pwm_out`, `comp_latch`, clock |
...
- **`comp_latch`** (digital): single pulse per bit, triggers comparator
...
3. **SAR pulses `comp_latch`** once after settling.  The comparator
